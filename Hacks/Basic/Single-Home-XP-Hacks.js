...
const DEFAULT_PCT = 0.99;			// This is the default 1-based percentage of money we want to grow on the server in a single pass
...
export async function main(ns) {
	...
	if (target == undefined) {
		...
		ns.tprint('INFO :    <pct> is the 1-based maximum percentage to grow from the target (Optional, default is 99%)');
		ns.tprint('INFO :');
...
async function Exploit(ns, server, pct, xpMode) {
	...
	// Determines if we have got to the grow part of the cycle
	let growOnce = false;
	...
	while (true) {
		...
		// Grow the server
		if ((money < maxMoney - maxMoney * MAX_MONEY_DRIFT_PCT) || xpMode) {
			ns.print('WARN: ***GROWING*** Money is getting low, we need ' + growThreads + ' threads to max it');
			let pids = await RunScript(ns, 'v1grow.js', server, growThreads, growOnce);
...
function MemoryReport(ns) {
	...
	if (used / total < 0.85)
		ns.print('WARN: The full grow cycle for this job is running with ' + pct + '% ram left. You could hack other servers, and/or increase the % of grow on this server.')
}
...
async function RunScript(ns, scriptName, target, threads, growOnce) {
	// Find all servers
	...
	if (growOnce && fired != threads) {
		ns.print('FAIL: There wasn\'t enough ram to run ' + threads + ' threads of ' + scriptName + ' (fired: ' + fired + '). It is recommended to either reduce the grow percentage or reduce memory usage from other scripts.');
	}
