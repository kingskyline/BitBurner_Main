...
async function getStockProfit(ns) {
    const symbols = ns.stock.getSymbols();
    let totalProfit = 0;

    for (const sym of symbols) {
        const position = ns.stock.getPosition(sym);
        const [shares, avgBuyPrice, shortShares, avgShortPrice] = position;

        // Profit from selling long positions
        if (shares > 0) {
            const currentPrice = ns.stock.getPrice(sym);
            totalProfit += shares * (currentPrice - avgBuyPrice);
        }

        // Profit from covering short positions
        if (shortShares > 0) {
            const currentPrice = ns.stock.getPrice(sym);
            totalProfit += shortShares * (avgShortPrice - currentPrice);
        }
    }
...
async function getHudData(ns, bitNode, dictSourceFiles, options) {
    ...
    const stockProfit = await getStockProfit(ns);
hudData.push(["Stock Profit", true, formatMoney(stockProfit), "Total profit from stock trading"]);
    ...
    {
        const playerInfo = await getNsDataThroughFile(ns, 'ns.getPlayer()');
        const city = playerInfo.city;
        const location = playerInfo.location.substring(0, 10); // Truncate long locations

        hudData.push(["City", true, city, "Current city where the player is located."]);
        hudData.push(["Location", true, location, "Current specific location within the city."]);
    }

    // Show Local Time
    {
        const localTime = new Date().toLocaleTimeString();
        hudData.push(["Local Time", true, localTime, "Current local time of your system."]);
    }
